...
if recalc:
    y1, x1 = calc_mean_sparse_degradation(Model_EWC_FIS, lr, 15., epoch_num, tries=10, backup_file='layers_by_w.dmp', sparse_by_weights=True)
    y2, x2 = calc_mean_sparse_degradation(Model_EWC_FIS, lr, 41., epoch_num, tries=10, backup_file='layers_by_fis.dmp')
    y3, x3 = calc_mean_sparse_degradation(Model_EWC_MAS, lr, 4.5, epoch_num, tries=10, backup_file='layers_by_mas.dmp')
    y4, x4 = calc_mean_sparse_degradation(Model_EWC_SI, lr, 0.25, epoch_num, tries=10, backup_file='layers_by_si.dmp')
    y5, x5 = calc_mean_sparse_degradation(Model_EWC_SIG, lr, 0.75, epoch_num, tries=10, backup_file='layers_by_sig.dmp')
else:
    y1, x1 = joblib.load('layers_by_w.dmp')
    y2, x2 = joblib.load('layers_by_fis.dmp')
    y3, x3 = joblib.load('layers_by_mas.dmp')
    y4, x4 = joblib.load('layers_by_si.dmp')
    y5, x5 = joblib.load('layers_by_sig.dmp')
